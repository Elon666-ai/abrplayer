...
*   **初始选流 (Cold Start)**：
    ...
    *   **测速兜底**：若无记忆，尝试通过下载静态文件测速（需配置）。静态文件地址: https://video.sdk.qcloudecdn.com/web/TXLivePlayer-1.3.4.min.js
    *   **安全兜底**：若测速失败，默认选择中间画质或指定的兜底流（如 `720p`），避免起播失败。
*   **混合带宽估算**：
    *   **被动统计 (Passive)**：在播放视频时，直接利用 WebRTC `getStats` 返回的实时接收码率 (`receiveKbps`) 作为带宽参考，**零额外流量消耗**。
    *   **主动探测 (Active)**：仅在当前处于最低画质（Audio 模式）且渴望升级时，才会启动低频文件下载测速，以探测网络上限。
...
| :--- | :--- | :--- | :--- | :--- |
| **0** | `{sitename}_audio` | 128 kbps | AAC | **兜底流**，纯音频，画面冻结 | bitrate<300kbps采用
| **1** | `{sitename}_economic` | 400 kbps | H.264 | 最低视频档 | bitrate<600kbps采用
| **2** | `{sitename}_standard` | 1000 kbps | H.264 | 通用高清 | bitrate<1500kbps采用
| **3** | `{sitename}_high` | 2000 kbps | H.264 | 最高画质 | bitrate>=1500kbps采用
...
1.  **推流端**：OBS 使用多路输出插件，同时推送 1080p, 720p, 540p 三路流。
2.  **服务端**：
